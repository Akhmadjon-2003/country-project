...
const GamesPanelBorder = () => {
  ...
  const renderAnswerCountry = () => {
    while(count > 0) {
      ...
      if(idCountryItem === -1 && countrys[randomNumber].borders) {
      ...
      }} py={3}>
        {
          countryItem.map((item) => (
            <GridItem key={item.area}>
              <Card onClick={() => checkedAnswer(item)} fontSize={'20px'} p={2} textAlign={'center'} border={'1px'} borderColor={cardBorderBg} bg={cardBg} cursor={'pointer'} _hover={{bg: cardHoverBg}}>{item.borders ? Object.values(item?.borders).toString().split(',').join('  ') : '-/-'}</Card>
            </GridItem>
          ))
        }
      </Grid>
...
export default GamesPanelBorder