  ...
  return (
    <Box p={5}>
      ...
      <Heading textAlign={'center'} fontSize={{
        base: '16px',
        lg: '26px',
      }} pb={2}>Where is this city the capital of?</Heading>
      ...
      }} py={3}>
        {
          countryItem.map((item) => (
            <GridItem key={item.area}>
              <Card onClick={() => checkedAnswer(item)} fontSize={'20px'} p={2} textAlign={'center'} border={'1px'} borderColor={cardBorderBg} bg={cardBg} cursor={'pointer'} _hover={{bg: cardHoverBg}}>{item.capital}</Card>
            </GridItem>
          ))
        }
      </Grid>
...
export default GamesPanelCountry