import { Badge, Box, Card, Divider, Grid, GridItem, Heading, HStack, Image, Text, useColorModeValue, Icon } from '@chakra-ui/react'
  ...
  return (
    ...
    <Box p={5}>
      ...
      <Heading textAlign={'center'} fontSize={{
        base: '16px',
        lg: '26px',
      }} pb={2}>Show the flag of this country?</Heading>

      <Box display={'flex'} alignItems={'center'} justifyContent={'center'} px={3} pt={1}>
        <Heading textAlign={'center'} fontSize={{
          base: '18px',
          lg: '30px'
        }}>{countryItem[randomAnswerNumber].name.common}</Heading>
      </Box>
      <Divider my={3}/>
      <Grid gridTemplateColumns={'1fr 1fr'} gap={6} px={{
        base: '1',
        lg: '100px',
      }} py={3}>
        {
          countryItem.map((item) => (
            <GridItem key={item.area}>
              <Card onClick={() => checkedAnswer(item)} fontSize={'20px'} textAlign={'center'} border={'1px'} borderColor={cardBorderBg} bg={cardBg} cursor={'pointer'} _hover={{bg: cardHoverBg}}><Image h={{
                base: '100px',
                lg: '200px',
              }} rounded={'md'} src={item?.flags.png}/></Card>
            </GridItem>
          ))
        }
      </Grid>
...
export default GamesPanelFlagsName